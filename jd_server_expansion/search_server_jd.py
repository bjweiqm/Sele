...
"""
@software: PyCharm
@file: search_server_jd.py
@time: 2016/10/13 16:48
"""
...
class SearchTestServer():

    def __init__(self):
        self.lis = []
        self.driver = webdriver.Chrome(r'D:\Item\MOBILE_M\template\chromedriver.exe')
        self.driver.get(ReadConfig('url', 'url'))

    def test_getText(self, path):
        sle =  self.driver.find_element_by_xpath(path).text
        print sle.strip()
        return sle.strip()
